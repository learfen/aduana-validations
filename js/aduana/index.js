...
import { Names , Phone } from '../typesCustom.js'
...
Object.prototype.assignInterface = function ( fn, parent ){
    ...
    fn.interface = args.map( item => {
        ...
        eval( `
            try{
                interfaceFind = ${interfaceName}; 
                classFind = ${interfaceName.replace('$','')}
            }catch( error ){}
            `)

        let typeClass 
        eval(`typeClass = typeof ${className}()`)
        if(typeClass != 'function'){
            ...
            eval('classFind = '+className.replace('$',''))
            typeClass='function'
        }
